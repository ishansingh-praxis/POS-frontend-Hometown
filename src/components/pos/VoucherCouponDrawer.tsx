...
export default function VoucherCouponDrawer({
  ...
  return (
    <PosActionDrawer title="Voucher / Coupon" icon={<TicketPercent className="h-4 w-4" />} onClose={onClose}>
      ...
      <div className="rounded-2xl border border-teal-200 bg-gradient-to-br from-teal-50 to-emerald-50 p-4 space-y-3">
        <div className="flex items-center gap-2">
          <div className="h-9 w-9 rounded-xl bg-teal-600 text-white grid place-items-center">
            <Wallet className="h-4 w-4" />
          </div>
          <div>
            <div className="text-sm font-bold text-teal-950">Customer Voucher</div>
            <div className="text-xs text-teal-800/70">Store-credit voucher tied to the customer's wallet</div>
          </div>
        </div>
        ...
        {!appliedVoucher ? (
          <>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-teal-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-teal-200"
              />
              <button
                onClick={validate}
                disabled={loading}
                className="rounded-xl bg-teal-600 text-white px-4 py-2 text-sm font-bold hover:bg-teal-700 disabled:opacity-60 inline-flex items-center gap-2"
              >
            ...
            {pendingVoucher && (
              <div className="rounded-xl bg-white border border-teal-200 p-3 space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="font-bold text-teal-700">{pendingVoucher.voucherCode}</span>
                  <span className="text-teal-700">Balance: {formatINR(pendingVoucher.availableAmount)}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  ...
                  <input
                    ...
                    className="w-24 px-2 py-1 rounded border border-teal-200 text-right text-xs"
                  />
                </div>
                <button
                  onClick={apply}
                  disabled={applyAmount <= 0}
                  className="w-full rounded-lg bg-emerald-600 text-white py-2 text-xs font-black hover:bg-emerald-700 disabled:opacity-50"
                >
                  Apply to Bill
                </button>
              </div>
          ...
          <div className="rounded-xl bg-white border border-teal-200 p-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-sm font-bold text-teal-700">Voucher Applied: {appliedVoucher.voucherCode}</div>
              ...
              <div className="text-right">
                <div className="text-lg font-black text-teal-700">− {formatINR(voucherApplyAmount)}</div>
                <button onClick={onRemoveVoucher} className="text-xs px-2 py-1 rounded-lg bg-white border border-teal-200 text-teal-700 hover:bg-teal-100 inline-flex items-center gap-1 mt-1">
